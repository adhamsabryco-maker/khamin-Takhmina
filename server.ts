      ...
      players: [
        {
          ...
          level: getLevel(p1ServerPlayer ? p1ServerPlayer.xp : (match.p1.xp || 0)),
        ...
        },
        {
          ...
          level: getLevel(p2ServerPlayer ? p2ServerPlayer.xp : (match.p2.xp || 0)),
...
io.on("connection", (socket) => {
    ...
    socket.on("get_player_data", (data, callback) => {
      ...
      if (player && callback) {
        ...
        if (!isAuthorized) {
            callback({ error: 'عذراً، هذا الحساب محمي. يرجى إدخال كلمة المرور (Secret Token) الصحيحة أو الدخول من جهازك الأصلي.' });
            return;
        }
